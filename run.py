...
def collect_data_genetic(m,n):
    ...
    """
    wb1 = openpyxl.load_workbook("lattice_table_greedy_double_slicing_rloo" + str(n)+".xlsx") # Open greedy algorithm spreadsheets
    wb2 = openpyxl.load_workbook("lattice_table_greedy_double_slicing_loo" + str(n)+".xlsx")
    filename = "lattice_table_genetic_" + str(n) + '.xlsx'
    config_indexes = [] # To store the configuration indices of the genetic algorithm that returns the solution the quickest.
    shelfFile = shelve.open("evolution_config")  
    try:
        wb = openpyxl.load_workbook(filename) # Open the spreadsheet for the genetic algorithm.
    except:
        wb = openpyxl.Workbook()
    
    for i in range(m,12): # we stop at m == 11 just like the tables in the previous papers
         
        for k in range(3,i+n-1):
            if wb1["Sheet"].cell(i+1,k+1).value > wb2["Sheet"].cell(i+1,k+1).value: # Checking which of the version of the greedy algorithms have 
          # a greater value at this cell
            ...
            try: # Might raise an error if the cell does not have a value yet
                ...
                wb["Sheet"].cell(i+1,k+1).value = target    
            wb.save(filename)
            while run: # Keeps incrementing the target and stops only if it didn't find any solution.
            ...
            try:
                ...
                wb["Sheet"].cell(i+1,k+1).value = target    
            wb.save(filename)
            shelfFile['list_of_configs'] = config_indexes  # Store the config indices in a shelf file  
    wb.close()
    wb1.close()
    wb2.close()
...
def parallel_search(target,m,n,k):
  ...
  # Set the parameter values for the search function  
    ...
    config_index = -1 # Configuration index to be returned if none of the parallel searches finds the solution
    ...
    with ProcessPool() as executor:
        ...
        while run:
            ...
            if tasks[-1].done():
                ...
                try:
                    print(type(result)) # checking whether result has already been assigned. This will raise an error if result has not yet been
                # assigned
                except:
                    result = tasks[0].result() # if result was not assigned, then none of the tasks found the solution.
                  # Don't put 'pass' here because it will create an error(bug) on the next line
                if result.fitnesses[result.bfi]!=9999: # important because even if task[-1].done() returns True, it might return true just because 
                 # it was cancelled. 
...
def greedy_t(m,n,k,paths,reverse):
    ...
    """
    solutions = []
    pati = paths.copy() # We use copy here because we don't want pati and paths to have the same reference
    max_indexes = (-2,-2) # This stores the indices of the paths we removed that returned the greatest result from greedy()
    max = greedy(m,n,k,pati,reverse) # we run the greedy() first with all the paths
    solutions.append(max)
    for i in range(len(paths)):
        patos = paths.copy() #paths, not pati because pati might have been been reversed inside greedy()
        patos.pop(i) # We remove the current path
        for j in range(len(patos)):
            patz = patos.copy()
            patz.pop(j)
            amax = greedy(m,n,k,patz,reverse) # Run the greedy with two paths popped out
            if amax>max:
                max = amax
                if j>=i:
                    max_indexes = (i,j+1)
...
def collect_data_greedy(m,n):
    ...
    No value returned.
    """
    filename1 = "lattice_table_greedy_double_slicing_rloo" + str(n) + '.xlsx' # spreadsheet name for running the greedy algorithm in reverse lexicographical order
    try:
        wb1 = openpyxl.load_workbook(filename1)
    except:
        wb1 = openpyxl.Workbook()
    filename2 = "lattice_table_greedy_double_slicing_loo" + str(n) + '.xlsx' # spreadsheet name for running the greedy algorithm in lexicographical order
    try:
