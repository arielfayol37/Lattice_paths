 ...
 """
    def __init__(self, num_sequences, m, n, k, paths, len_paths, empty=False):
        ...
        *paths is a paramter, but we don't need paths here though. However, the program was originally written this way. 
        ...
        self.take_paths = [i for i in range(len_paths)] # list of all paths(indices actually)
        #self.poison()
    ...
    def _generate_sequences(self, paths, len_paths, empty):
        ...
        if not empty: #Generate valid paths
            for i in range(self.num_sequences):
                ...
                r = random.randint(0, len_paths - i - 1) # Choose a random index from the list of available paths.
               
                self.sequences.append(self.take_paths[r]) # Add the new sequence to the genome, and remove the chosen path from the list of available paths.
    ...
    def fitness(self, dict_equivalences, penalty_indexes=True):
        ...
        if penalty_indexes: # If the penalty_indexes parameter is True, calculate the penalty and the penalty index.
            penalty_index = [] #To store the indices of Sequences that are k-equivalent.
          
            for i in range(self.num_sequences):
                for j in range(i + 1, self.num_sequences):
                    if self.sequences[j] in dict_equivalences[str(self.sequences[i])]: # are k-equivalent
                        penalty+=1
    ...
    def mutate(self, dict_equivalences):
        ...
        index_eq = self.fitness(dict_equivalences)[-1] # Get the indices of k-equivalent sequences.
        i = random.randint(0, len(self.take_paths) - 1) # Choose a random index from the list of available paths.

        if index_eq: # If the penalty index is not empty, choose a random index from the penalty index.
            ...
            self.sequences[r] = self.take_paths[i] # Replace the sequence at the chosen index with a new sequence using the chosen path.
    ...
    def nmutate(self, dict_equivalences):
      ...
      """
        ...
        index_eq = len(self.fitness(dict_equivalences)[-1]) # this list will just be used to check if mutation is needed. If empty, then Genome is perfect.
