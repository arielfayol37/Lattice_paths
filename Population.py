...
class Population:
    ...
    def __init__(
        ...
        self.c_fitnesses = (
            []
        )  # A list to contain the new Genome fitnesses during mating

        self.p_mating = []  # A list to containing the mating probabilities
        self.crossover_feq = 0.7  # Crossover frequency
        self.indexes = (
            []
        ...
        self.roulette_ready = (
            False  # Boolean to determine whether the mating pool is ready
        )
        self.best_fitness = 0
        ...
        self.equivalences = (
            dict()
        )  # To store the paths indices as keys and the indices of paths they are equivalent to as values
    ...
    def compute_equivalences(self):
        ...
        for i in range(self.l):
            foo = self.equivalences.setdefault(
                str(i), set([i])
            )  # we don't need the return value of setdefault()
        ...
        for i in range(self.l):  # We compare each path to every other
            for j in range(i + 1, self.l):
                ...
                ):  # For every term(edge) a in path i and b in path j.
                    if a == b:
                        equi += 1
    ...
    def initialize(self):
        ...
        for r in range(50):
            ...
            new_genome = Genome(
                self.num_genes, self.m, self.n, self.k, self.paths, self.l, True
            )  # Creating individual with no paths yet
        ...
        for z in range(int(new_indi_coef * self.size)):
            ...
            new_genome = Genome(
                self.num_genes, self.m, self.n, self.k, self.paths, self.l
            )
    ...
    def cal_div(self, sort=True):
        ...
        for indi in self.individuals:
            self.divergences.append(
                1
                - (sum([self.distribution[str(i)] for i in indi.sequences]))
                / denominator
            )

        assert len(self.divergences) == len(
            self.individuals
        ), "difference in size of divergences and individuals list in cal_div()"
        av_div = sum(self.divergences) / len(self.individuals)
        self.av_pop_divergences.append(av_div)

        assert len(self.fitnesses) == len(
            self.individuals
        ), "difference in size of fitnesses and individuals list in cal_div()"
        av = sum(self.fitnesses) / len(self.fitnesses)
        try:
            if (
                av == self.av_pop_fitnesses[-30]
            ):  # If the current average fitness is the same as that of 30 generations ago, then
    ...
    def battle(self, mode="non_bias_random"):
        ...
        if mode == "non_bias_random":
            while len(self.individuals) > self.max_size:
                index_1 = random.randint(
                    0, len(self.individuals) - 1
                )  # we subtract 1 bescause the randint() function includes the bounds
                index_2 = random.randint(0, len(self.individuals) - 1)
                while (
                    index_1 == index_2
                ):  # This might waste time or even lead to an infinite loop (bug) if the population size is too small
                    index_2 = random.randint(0, len(self.individuals) - 1)
    ...
    def parent_pick(self, mode="roulette"):
        ...
        if mode == "roulette":
            ...
            if not self.roulette_ready:
                ...
                v_prime = self.fm * (
                    1 - v
                )  # weight for fitnesses and divergences respectively, when computing the mating probability
                ...
                n = int(
                    len(self.individuals) * (2 * self.crossover_feq + 0.2)
                )  # Make sure the number here is a little more than twice than the mating rate.
                # otherwise the mating pool will be empty

                self.pool.clear()
                assert len(self.indexes) == len(
                    self.p_mating
                ), "Different size between self.indexes and self.p_mating in parent_pick()"
            ...
            rand1 = random.randint(
                0, len(self.pool) - 1
            )  # might raise an error(bug) if pool is empty
            ...
            ):  # might lead to infinite loop(bug) if population size too small
                parent_2_index = random.randint(0, len(self.individuals) - 1)
            return (parent_1_index, parent_2_index)
    ...
    def mating(self, mode="random_random"):
        ...
        self.just_initialized = False
        l = len(self.individuals)
        for j in range(int(self.crossover_feq * l)):
            co_coef = int.from_bytes(os.urandom(8), byteorder="big") / (
                (1 << 64) - 1
            )  # randomly generate cross over coeficient to determing where cross over will take place
            ...
            if parent_1_index != parent_2_index:
                new_child_1 = Genome(
                    self.num_genes, self.m, self.n, self.k, self.paths, self.l, True
                )  # create empty individual (without genes)
                new_child_2 = Genome(
                    self.num_genes, self.m, self.n, self.k, self.paths, self.l, True
                )
                new_child_3 = Genome(
                ...
                new_child_2.mutate(
                    self.equivalences
                )  # mutate the individual by swapping a gene that is k-equivalent to another(or others) with another
                ...
                if j % 100 == 0:  # mutate child 1 and 3 for every 100th mating
                    new_child_1.nmutate(
                        self.equivalences
                    )  # This randomly changes a gene, even if it was k-distinct to all other paths.
                    new_child_3.smutate(
                        self.equivalences, self.l
                    )  # Same as smutate, but in addition, it might be swapped for a gene already present in the chromosome
    ...
    def find_best(self):
        ...
        """
        assert len(self.individuals) == len(
            self.fitnesses
        ), "individuals list and fitnesses list are different in size in self.find_best()"
        self.min_fitness = self.fitnesses[0]
        for i in range(len(self.fitnesses)):
            if (
                self.fitnesses[i] >= self.best_fitness
            ):  # it as to be greater or equal to for cal_div to work properly
    ...
    def check(self, mode, test=True):
        ...
        if (
            mode == "roulette" or mode == "random_random"
        ):  # This is option is not used anymore or it is used only for testing
            ...
            if test:
                for s in range(len(self.individuals)):
                    try:
                        assert (
                            self.individuals[s].fitness(self.equivalences, False)
                            == self.fitnesses[s]
                        )
                    except:
                        raise Exception(
                            "Difference between real fitness and calculated fitness in self.check()"
                        )
            ...
            if self.best_fitness == 9999:
                ...
                assert (
                    self.individuals[self.bfi].fitness(self.equivalences, False)
                    == self.best_fitness
                )
                logging.info(
                    "(num_solutions: {}, m: {}, n: {}, k: {}) \n".format(
    ...
    def evolve(self, mode, kill_mode="non_bias_random"):
        ...
        for i in range(1, self.eons):
            ...
            if (i) % 50 == 0:
                found = (
                    self.initialize()
                )  # After every 50 generations, we refill the population to maintain some gene diversity
                self.sorted = False
    ...
    def visualize_evolution(self):
        ...
        plt.plot(
            range(len(self.max_fitnesses)),
            self.max_fitnesses,
            color="blue",
            label="Best fitness",
        )
        plt.plot(
            range(len(self.av_pop_fitnesses)),
            self.av_pop_fitnesses,
            color="red",
            label="Av pop fitness",
        )
        plt.plot(
            range(len(self.av_pop_divergences)),
            self.av_pop_divergences,
            color="green",
            label="Av pop divergences",
        )
        plt.legend()
